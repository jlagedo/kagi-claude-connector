...
- **The Kagi v1 contract is verified** — both against the live API (`scripts/smoke.mjs` does a real `kagi_search` in prod) and the official docs. `POST /search` with `format:"markdown"` returns markdown as the raw body, `POST /extract` with `format:"json"` returns `{data:[{markdown}]}`, and auth is `Authorization: Bot <key>` (the curl example at [help.kagi.com/kagi/api/search.html](https://help.kagi.com/kagi/api/search.html) uses `Bot`). `KAGI_AUTH_SCHEME` (a var in `wrangler.jsonc`) is an escape hatch — flip it to `Bearer` only if a future key 401s under `Bot`.
